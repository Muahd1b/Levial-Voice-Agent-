...
- Config-driven architecture; `voice_loop.py` contains the current MVP pipeline with rolling session memory.
...
### Coding Guidelines
- Prefer config-driven behavior over hard-coded constants; surface new toggles in PRD/config docs.
- Record mic audio at 16 kHz mono unless requirements change.
- Log key steps (recording start, transcription result, LLM response, TTS completion).
- Add focused comments only where intent is not obvious.

### Testing Instructions
- Run `python voice_loop.py`, verify end-to-end audio roundtrip.
...
3. **Verify full loop**: execute `python voice_loop.py` and confirm microphone → TTS roundtrip.
